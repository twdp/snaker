...
type IProcessService interface {
	...
	// 根据主键ID获取流程定义对象
	GetProcessById(id int64) (*entity.Process, error)
	...
	DeployByCreator(input string, creator string) (int64, error)
	...
	// 解析内容成model
	ParseModel(process *entity.Process) (*ProcessModel, error)
}
...
func (p *ProcessService) GetProcessById(id int64) (*entity.Process, error) {
	panic("implement me")
}
...
func (p *ProcessService) DeployByCreator(input string, creator string) (int64, error) {
	if model, err := p.Parser.ParseXml(input); err != nil {
		return 0, err
	} else {
		fmt.Println(model)
		return 0,nil
...
func (p *ProcessService) ParseModel(process *entity.Process) (*ProcessModel, error) {
	if model, err := p.Parser.ParseXml(process.Content); err != nil {
		return nil, err
	} else {
