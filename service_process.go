...
type ProcessService struct {
	...
	NameCache              map[int64]string       //ProcessName缓存，KEY为ID
...
func (p *ProcessService) InitProcessService() {
	...
	p.NameCache = make(map[int64]string)
...
//部署Process
func (p *ProcessService) Deploy(input []byte, creator string) int64 {
...
//重新部署Process
func (p *ProcessService) ReDeploy(id int64, input []byte) {
	process := &Process{}
...
//卸载部署
func (p *ProcessService) UnDeploy(id int64) {
	process := &Process{}
	success := process.GetProcessById(id)
...
//根据ID得到Process
func (p *ProcessService) GetProcessById(id int64) *Process {
...
func (p *ProcessService) GetProcessByVersion(name string, version int) *Process {
	...
	if process == nil {
		...
		success := process.GetProcessByNameAndVersion()
		if success {
			p.Cache(process)
			return process
		} else {
